...
import com.limelight.components.detectMotionEvents
import com.limelight.components.detectMotionEventsAsList


/**
 * Create a modifier for processing pointer motion input within the region of the modified element.
 *
 * After [AwaitPointerEventScope.awa itFirstDown] returned a [PointerInputChange] then
 ...
 *
 * To prevent other pojinter functions that call [awaitFirstDown]
...
fun Modifier.pointerMotionEvents(
    ...
    key1: Any? = Unit) = this.then(Modifier.pointerInput(key1) { detectMotionEvents(
            onDown,
            onMove,
            onUp,
